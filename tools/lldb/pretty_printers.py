...
MAX_REASONABLE_CAPACITY = 1 << 24  # 16M slots — generous for real hashmaps, bounds worst-case hang
MAX_REASONABLE_LEN      = 1 << 24  # elements, for slices
...
class zig_Slice_SynthProvider:
    ...
    def update(self):
        ...
        try:
            ...
            self.elem_size = self.elem_type.size or 1

            addr = self.ptr.unsigned
            if addr > page_size:
                length = self.value.GetChildMemberWithName('len').unsigned
                if 0 < length <= MAX_REASONABLE_LEN and _readable_span(self.value.process, addr, length * self.elem_size):
                    self.len = length
        except:
            self.len = 0
...
def zig_String_decode(value, offset=0, length=None):
    try:
        ...
        length = length if length is not None else value.GetChildMemberWithName('len').unsigned
        if length <= 0:
            return ''
        truncated = length > MAX_REASONABLE_STRLEN
        if truncated:
            length = MAX_REASONABLE_STRLEN
        if not _readable_span(value.process, ptr.unsigned, offset + length):
            return None
        data = ptr.GetPointeeData(offset, length)
        b = bytes(data.uint8)
        b = b.replace(b'\\', b'\\\\')
        b = b.replace(b'\n', b'\\n')
        b = b.replace(b'\r', b'\\r')
        ...
        s = s if s.isprintable() else ''.join((c if c.isprintable() else '\\x%02x' % ord(c) for c in s))
...
class std_HashMapUnmanaged_SynthProvider:
    ...
    def update(self):
        try:
            ...
            hdr_addr = self.metadata.deref.load_addr - hdr_type.size
            if not _readable_span(self.value.process, hdr_addr, hdr_type.size):
                return

            hdr = self.metadata.CreateValueFromAddress('header', hdr_addr, hdr_type)
            ...
            # must be 0 or a power of two (HashMapUnmanaged always doubles)
            if self.capacity != 0 and (self.capacity & (self.capacity - 1)) != 0:
                self.capacity = 0
                return

            # sanity bound BEFORE we touch any memory sized by capacity
            if self.capacity > MAX_REASONABLE_CAPACITY:
                self.capacity = 0
                return

            # the metadata byte array itself must actually be backed by `capacity` readable bytes
            if not _readable_span(self.value.process, self.metadata.unsigned, self.capacity):
                self.capacity = 0
                return

            self.indices = tuple(i for i, v in enumerate(self.metadata.GetPointeeData(0, self.capacity).sint8) if v < 0)
        except:
            self.capacity = 0
            self.indices = tuple()
    def has_children(self): return True
...
def __lldb_init_module(debugger, _=None):
    ...
    add(debugger, category='zig.lang', regex=True, type='^\\[\\](const )?u8$', identifier='zig_String', summary=True)
